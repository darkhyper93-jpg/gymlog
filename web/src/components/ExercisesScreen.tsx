...
import { Button, Card, IconButton, Modal, Spinner, StateView } from './ui';
...
// Agrupa los ejercicios por grupo muscular, respetando el orden de MUSCLE_GROUPS, después los
// custom (orden de creación) y dejando "Otros" al final para los sin grupo.
function groupExercises(exercises: Exercise[], customGroups: MuscleGroup[]): Section[] {
  ...
  for (const g of customGroups) {
    const items = byKey.get(g.name);
    if (items && items.length > 0) sections.push({ key: g.name, label: g.name, items });
  }
...
export function ExercisesScreen({ onSelect }: { onSelect: (exercise: Exercise) => void }) {
  ...
  const { groups: customGroups, add: addGroup, remove: removeGroup } = useMuscleGroups();
  const [adding, setAdding] = useState(false);

  const sections = useMemo(() => groupExercises(exercises, customGroups), [exercises, customGroups]);
...
function ExerciseCard({
  ...
  onEdit: (id: string, input: { name?: string; muscleGroup?: string }) => Promise<void>;
